//! FXAA pass (§13). A fullscreen triangle that samples the tonemapped LDR image
//! and writes the swapchain, softening geometry edges at a cost that tracks
//! resolution rather than scene complexity — the cheap alternative to MSAA.
//!
//! Structurally a twin of `TonemapPass`: one descriptor set per frame-in-flight
//! pointing at the source image, re-pointed by `update` when that view changes
//! across a resize.
...
impl FxaaPass {
    pub fn new(renderer: &Renderer) -> Self {
        ...
        let sets = unsafe {
            device
                ...
                .expect("allocate fxaa sets")
        };

        let vert = load_shader(&device, spv!("fullscreen.vert"));
        let frag = load_shader(&device, spv!("fxaa.frag"));
        let stages = [
    ...
    /// Point this frame's descriptor at the current LDR view. No-op if unchanged
    ...
    pub fn update(&mut self, frame: usize, ldr_view: vk::ImageView, sampler: vk::Sampler) {
        if self.bound_views[frame] == ldr_view {
            return;
        }
        ...
        self.bound_views[frame] = ldr_view;
    }
    ...
    pub fn draw(&self, cmd: vk::CommandBuffer, extent: vk::Extent2D, frame: usize) {
        ...
        let push = unsafe { std::slice::from_raw_parts(inv.as_ptr() as *const u8, 8) };
        unsafe {
            self.device
